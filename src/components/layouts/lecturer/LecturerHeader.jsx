...
function HeaderMeta({ title, subtitle, breadcrumbs }) {
  ...
  return (
    <div>
      <h1 className="text-slate-900 font-black text-[18px] leading-none">{title}</h1>

      {hasBreadcrumbs ? (
        <div className="flex items-center text-[11px] text-slate-400 gap-1 mt-1 flex-wrap">
          {breadcrumbs.map((item, index) => (
            <React.Fragment key={`${item.label}-${index}`}>
              {item.to ? (
                ...
                <span className="text-[#F37021] font-semibold">{item.label}</span>
              )}
